...
def write_partFile(part_num, Dim, nparts, run_id):
    pname = f"results/{run_id}/comm" + str(nparts) + ".txt"
...
def write_resultFile(result, run_id):
    print('\nResult info:\n', result)

    result["total_solns"] = int(result["total_solns"])
    result["num_occ"] = int(result["num_occ"])
    result["wall_clock_time"] = str(result["wall_clock_time"])
    resultFile = open(f"results/{run_id}/result.json", "w")
    json.dump(result, resultFile)

    resultFile.close()


def showClusters(part_number, graph, run_id):
    # drawing
    color = {
        0: 'red',
        1: 'blue',
    ...
    size = float(len(set(partition.values())))  # noqa: F841
    ...
    for com in set(partition.values()):
        ...
        nx.draw_networkx_nodes(graph, pos, list_nodes, node_size=80, node_color=color[com])

    nx.draw_networkx_edges(graph, pos, alpha=0.5)
    plt.savefig(f"results/{run_id}/clusters.png", dpi=600)
