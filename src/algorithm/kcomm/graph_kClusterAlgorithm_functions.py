...
def runDwave(Q, num_nodes, k, embedding, qsize, run_label, run_profile, result):
    ...
    result['wall_clock_time'] = str(wtime)
...
def runDwaveDirect(Q, k, embedding, run_label, run_profile, result):
    ...
    wtime = dt.datetime.now() - t0
    result['wall_clock_time'] = str(wtime)
    print('\nPercentage of samples with high rates of breaks (> 0.10) is ', np.count_nonzero(solution.record.chain_break_fraction > 0.10) / num_reads * 100)
...
def runDwaveHybrid(Q, num_nodes, k, sub_qsize, run_label, run_profile, result):
    ...
    iteration = hybrid.Race(
        ...
        hybrid.EnergyImpactDecomposer(size=sub_qsize, rolling=True, rolling_history=0.15) | QPUSubSamTime | hybrid.SplatComposer()
